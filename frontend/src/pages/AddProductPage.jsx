...
import { categories } from '../data/offersData'; // ❌ cities убраны из импорта
...
const AddProductPage = () => {
  ...
  // 🆕 Состояния для городов с сервера
  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        ...
        // Преобразуем в формат, удобный для UI: { value: id, label: name }
  ...
  const handleSubmit = async (event) => {
    ...
    if (!formData.name.trim()) { setError('Заполните название продукта.'); return; }
    if (!formData.category) { setError('Выберите категорию.'); return; }
    if (!formData.short_description.trim()) { setError('Добавьте короткое описание для карточки.'); return; }
    if (!formData.cities.length) { setError('Выберите хотя бы один город получения.'); return; }

    const token = localStorage.getItem('auth_token');
    if (!token) { setError('Нужна авторизация для добавления продукта.'); return; }
    ...
    // Отправляем ID городов, а не названия
    ...
    try {
      const response = await fetch('http://localhost:8000/server_cm/products/new/', {
        method: 'POST',
        headers: { 'Authorization': `Token ${token}` },
        body: payload,
      });
      const result = await response.json();
      if (!response.ok) { setError(result.error || 'Не удалось добавить продукт.'); return; }
    ...
    <div className="add-product-page">
      <section className="add-product-intro">
        <div>
          ...
          <p>Соберите карточку предложения с фото, кратким описанием и городами выдачи. После сохранения продукт сразу попадёт в каталог.</p>
        </div>
      </section>
        ...
        <form className="add-product-form" onSubmit={handleSubmit}>
          <div className="add-product-form__grid">
            <div className="photo-upload">
              <div className={`photo-upload__preview ${previewUrl ? 'has-image' : ''}`} onClick={handleChooseFile}>
                {previewUrl ? (
            ...
            <label className="form-field">
              <span>Название</span>
              <input type="text" name="name" value={formData.name} onChange={handleFieldChange} placeholder="Например, Кредит наличными" />
            </label>
            ...
            <div className="cities-section">
              ...
              {/* 🆕 Обработка загрузки и ошибок списка городов */}
              {isLoadingCities ? (
                ...
                <>
                  <div className="cities-grid">
                    {cities.map((city) => {
                      ...
                      return (
                        <label key={city.value} className={`city-chip ${isSelected ? 'is-selected' : ''}`}>
                          <input type="checkbox" checked={isSelected} onChange={() => handleCityToggle(city.value)} />
            ...
            <label className="form-field">
              ...
              <textarea name="short_description" value={formData.short_description} onChange={handleFieldChange} rows="4" placeholder="Короткий текст для карточки в каталоге" />
            </label>

            <label className="form-field">
              <span>Исходная ссылка</span>
              <input type="url" name="source_url" value={formData.source_url} onChange={handleFieldChange} placeholder="https://bank.ru/product" />
            </label>

            <label className="form-field form-field--full">
              <span>Описание</span>
              <textarea name="description" value={formData.description} onChange={handleFieldChange} rows="7" placeholder="Подробно опишите условия, преимущества и ограничения продукта" />
            </label>
          </div>
          ...
          <div className="form-footer">
            <Link to="/catalog" className="form-footer__link">Перейти в каталог</Link>
